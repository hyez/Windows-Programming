...
int _tmain (int argc, LPTSTR argv[])
{
	...
	mblock.mguard = CreateMutex (NULL, TRUE, NULL);
	...
	Status = WaitForSingleObject (consume_h, 0);
	if () 
		ReportError (_T("Failed waiting for consumer thread"), 3, TRUE);
	Status = WaitForSingleObject (produce_h, 0);
	if () 
...
/* Producer thread - Create new messages at random intervals */
{
	...
	while (!mblock.f_stop) {
		...
		__finally { ReleaseMutex (mblock.mready); }
	}
	return 0;
}
...
DWORD WINAPI consume (void *arg)
{
	...
	while (!ShutDown) { /* This is the only thread accessing stdin, stdout */
		...
		if (command == _T's')) {
			WaitForSingleObject (mblock.mguard, 1000);
			ShutDown = mblock.f_stop = 0;
