/* Job management utility functions. DELIBERATELY BROKEN VERSION */
/* These functions illustrate file locking. */
/*  As implemented, these functions use LockFileEx
	and will not work under Windows 95.
	Use LockFile for Windows 95 operation, in which case
	...
	Return -1 in case of failure. */
{
	...
	RegionStart.Offset = /*??*/;
	RegionStart./*??*/ = 0;
	FsLow = GetFileSize (hJobData, &/*??*/h);
	LockFileEx (hJobData, LOCKFILE_/*??*/,
		0, FsLow + /*??*/, 0, &RegionStart);
	...
	while (ReadFile (hJobData, &JobRecord, /*??*/, &nXfer, NULL) && (nXfer > 0)) {
		if (JobRecord.ProcessId == 0) break;

		/* Also use this slot if corresponding job has ended. */

		hProcess = OpenProcess (/*??*/, FALSE, JobRecord./*??*/);
	...
	if (nXfer != 0)		/* Not at end of file. Back up a record. */
		SetFilePointer (hJobData, -(/*??*/) SJM_JOB, NULL, FILE_CURRENT);
					/* Enter the new Job record. */
	JobRecord.ProcessId = pProcessInfo->/*??*/;
	_tcsnccpy (JobRecord./*??*/, Command, MAX_PATH);
	WriteFile (hJobData, &/*??*/, SJM_JOB, &nXfer, NULL);
	}

	_finally {		/* Release the lock on the file. */
		UnlockFileEx (hJobData, /*??*/, /*??*/, 0, &/*??*/);
	...
	In the process remove all jobs that no longer exist in the system. */
{
	...
	LockFileEx (hJobData, /*??*/, 0, FsLow, FsHigh, &RegionStart);

	while (ReadFile (hJobData, &/*??*/, SJM_JOB, &nXfer, NULL) && (nXfer > 0))
	_try {
		JobNumber++;
		if (JobRecord.ProcessId == 0) continue;
					/* Obtain process status. */
		hProcess = OpenProcess (/*??*/S, FALSE, JobRecord.ProcessId);
		if (hProcess != NULL)
		...
		if (hProcess == NULL) { /* Back up one record. */
			SetFilePointer (hJobData, -(LONG)/*??*/, NULL, /*??*/);
...
/* Create the name of the job management file in the temporary directory. */
{
	...
	DWORD UNSize = MAX_PATH, TPSize = MAX_PATH;
 	...
 	if (!GetUserName (/*??*/, &UNSize))
		return FALSE;
	if (GetTempPath (/*??*/, TempPath) > TPSize)
		return FALSE;
