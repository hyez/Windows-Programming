		...
		The list must be terminated with NULL. */
{
	...
	while ((pMsg = va_arg (pMsgList, const char *)) != NULL) {
		msgLen = lstrlenA(pMsg);
		if (!WriteConsoleA(hOut, pMsg, msgLen, &count, NULL)
...
BOOL ConsolePrompt (const char *pPromptMsg, char *pResponse, DWORD maxChar, BOOL echo)
	...
	echo:		Do not display the user's response if this flag is FALSE. */
{
	...
	BOOL success;
	hIn = CreateFileA("CONIN$", GENERIC_READ | GENERIC_WRITE, 0,
			NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	hOut = CreateFileA("CONOUT$", GENERIC_WRITE, 0,
			NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	success =  SetConsoleMode (hIn, ENABLE_LINE_INPUT | echoFlag | ENABLE_PROCESSED_INPUT)
			...
			&& ReadConsoleA (hIn, pResponse, maxChar - 2, &charIn, NULL);
