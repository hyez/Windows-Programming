	...
	This program modifies Program 8-1, which used processes. */
{
	...
	for (iThrd = 0; iThrd < argc - 2; iThrd++) {
		...
		wcscpy_s(gArg[iThrd].targv[1], argv[1]); /* Pattern. */
		wcscpy_s(gArg[iThrd].targv[2], argv[iThrd + 2]); /* Search file. */
	...
	while (threadCount > 0) {
		...
		wcscpy_s(gArg[iThrd].targv[3], gArg[threadCount - 1].targv[3]);
		wcscpy_s(gArg[iThrd].targv[2], gArg[threadCount - 1].targv[2]);
		threadCount--;
	}
	free(tHandle);
	free(gArg);
