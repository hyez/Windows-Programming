...
int _tmain(int argc, LPTSTR argv[])
{
	...
	if (NULL == hSem)
		ReportError("Failed creating semaphore.", 1, TRUE);
	/* Create the mutex */
	if ((hMutex = CreateMutex(NULL, FALSE, NULL)) == NULL)
		ReportError("Failed creating mutex.", 1, TRUE);
	...
	if (hWorkers == NULL || pThreadArgsArray == NULL)
		ReportError("Cannot allocate working memory for worker handles or argument array.", 2, TRUE);
